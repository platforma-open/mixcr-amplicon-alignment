...
const result = computed(() => (resultMap.value ? [...resultMap.value.values()] : []));
...
const columnDefs: ColDef<AmpliconAlignmentResult>[] = [
  ...
  createAgGridColDef<AmpliconAlignmentResult, string>({
    ...
    progress(cellData) {
      ...
      if (cellData === 'Not started' || cellData === 'Queued') {
        return {
          status: 'not_started',
          text: parsed.stage || 'Not started',
        };
      }
