...
watch(
  () => app.model.data.assemblingFeature,
  (v) => {
...
const assemblingFeatureSelection = computed<string>({
  get: () => {
    ...
    const v = app.model.data.assemblingFeature;
  ...
  set: (value: string) => {
    if (value === "custom") {
      ...
      if (presetAssemblingFeatures.includes(app.model.data.assemblingFeature ?? "")) {
        app.model.data.assemblingFeature = "";
      }
    } else {
  ...
  <PlAccordionSection label="Advanced Settings">
    ...
    <PlCheckbox v-model="disableLowQualityMapping">
      ...
      <PlTooltip class="info" position="top">
        <template #tooltip>
          Reduces memory usage on large datasets by dropping low-quality reads instead of mapping
          them to existing clonotypes.
        </template>
      </PlTooltip>
    </PlCheckbox>
    <PlNumberField
