import type { LibraryEntryDefinition, VAnchorPoints, JAnchorPoints } from '@platforma-open/milaboratories.mixcr-amplicon-alignment.model';
 ...
 */
export function extractCdr3FromDefinitions(
  entries: LibraryEntryDefinition[],
): string | undefined {
  const parts: string[] = [];
  ...
  for (const entry of entries) {
    ...
    const vCdr3 = entry.vSequence.substring(entry.vAnchorPoints.cdr3Begin, entry.vAnchorPoints.vEnd);
    const jCdr3 = entry.jSequence.substring(entry.jAnchorPoints.jBegin, entry.jAnchorPoints.fr4Begin);
...
export function recomposeVGene(regions: VRegions): { vSequence: string; vAnchorPoints: VAnchorPoints } {
  const vSequence = regions.fr1 + regions.cdr1 + regions.fr2 + regions.cdr2 + regions.fr3 + regions.vPartCdr3;
  let pos = 0;
  const fr1Begin = pos;
...
export function recomposeJGene(regions: JRegions): { jSequence: string; jAnchorPoints: JAnchorPoints } {
