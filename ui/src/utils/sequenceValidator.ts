...
export function validateLibrarySequence(sequence: string): SequenceValidationResult {
  ...
  if (translatedSequence.length === 0) {
    return {
      isValid: false,
      error: 'Translation resulted in empty protein sequence (possibly due to early stop codon)',
    };
  }

  // Check if translation contains unknown amino acids
  if (translatedSequence.includes('X')) {
    warnings.push('Translation contains unknown amino acids (X) due to invalid codons');
  }

  // Apply the regex validation: C[ACDEFGHIKLMNPQRSTVWY]+(?:[GAST])?[WFL]
  const validationRegex = /C[ACDEFGHIKLMNPQRSTVWY]+(?:[GAST])?[WFL]/;
  ...
  const jGeneStartNucleotides = patternEndNucleotides - 9; // 6 nucleotides before pattern end
